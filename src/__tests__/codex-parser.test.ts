...
describe('codex parser hardening', () => {
  ...
  it('derives the summary from response_item user rows in new rollouts (no event_msg user_message)', async () => {
    const home = makeCodexHome();
    writeRollout(
      home,
      path.join('sessions', '2026', '09', '05'),
      'rollout-2026-09-05T13-01-35-new-format-session-id.jsonl',
      [
        {
          timestamp: '2026-09-05T13:01:35.000Z',
          type: 'session_meta',
          payload: { id: 'new-format-session-id', cwd: '/tmp/project' },
        },
        {
          timestamp: '2026-09-05T13:01:36.000Z',
          type: 'response_item',
          payload: {
            type: 'message',
            role: 'user',
        ...
        {
          timestamp: '2026-09-05T13:01:37.000Z',
          type: 'response_item',
          payload: {
            type: 'message',
            role: 'user',
            content: [{ type: 'input_text', text: '帮我看看claude code源码里的snip机制' }],
          },
        },
      ],
    );
    ...
    // the harness-injected row is skipped, the real prompt becomes the summary
    expect(sessions[0].summary).toBe('帮我看看claude code源码里的snip机制');
  });
