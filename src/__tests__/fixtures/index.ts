...
/**
 * Create a temporary directory with a Qoder transcript fixture.
 * Mirrors the IDE layout: <root>/projects/<cwd-slug>/transcript/task-*.session.execution.jsonl
 */
export function createQoderFixture(): FixtureDir {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'test-qoder-'));
  const projectsDir = path.join(root, 'projects');
  const transcriptDir = path.join(projectsDir, '-home-user-project', 'transcript');
  fs.mkdirSync(transcriptDir, { recursive: true });

  const sessionId = 'task-test-qoder-1.session.execution';
  const lines = [
    JSON.stringify({
      type: 'session_meta',
      sessionId,
    ...
    JSON.stringify({
      ...
      message: {
        role: 'assistant',
        content: [
          { type: 'thinking', thinking: 'The user wants the auth bug fixed. I should read login.ts first.' },
          { type: 'text', text: 'I found the issue in login.ts. The token validation was missing.' },
          { type: 'tool_use', id: 'toolu_test_qoder_1', name: 'Edit', input: { file_path: 'login.ts' } },
        ],
      },
    }),
    JSON.stringify({
      ...
      timestamp: '2026-01-15T10:00:05.500Z',
      cwd: '/home/user/project',
      data: { command: 'hook_entry --agent-type=qoder' },
    }),
    JSON.stringify({
      type: 'user',
      sessionId,
      ...
      message: {
        ...
        content: [
          {
            type: 'tool_result',
            tool_use_id: 'toolu_test_qoder_1',
    ...
    JSON.stringify({
      ...
      message: {
        role: 'assistant',
        content: [{ type: 'text', text: 'The fix is applied. Token validation now runs on every login.' }],
      },
    }),
  ];
...
/**
 * Create a temporary directory with a QoderWork session fixture.
 * Mirrors the paired layout: <root>/projects/<cwd-slug>/<uuid>-session.json + <uuid>.jsonl
 */
export function createQoderWorkFixture(): FixtureDir {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'test-qoderwork-'));
  const projectsDir = path.join(root, 'projects');
  const sessionDir = path.join(projectsDir, '-home-user-project');
  fs.mkdirSync(sessionDir, { recursive: true });

  const sessionId = '00000000-0000-0000-0000-000000000201';
  fs.writeFileSync(
    path.join(sessionDir, `${sessionId}-session.json`),
    JSON.stringify({
      id: sessionId,
      parent_session_id: '',
      title: 'Fix auth bug',
      ...
      cost: 0.01,
      created_at: Date.parse('2026-01-15T10:00:00.000Z'),
      updated_at: Date.parse('2026-01-15T10:00:07.000Z'),
  ...
  const lines = [
    ...
    JSON.stringify({
      ...
      message: {
        ...
        stop_sequence: null,
        content: [
          { type: 'thinking', thinking: 'The user wants the auth bug fixed. I should inspect login.ts.' },
          { type: 'text', text: 'I found the issue in login.ts. The token validation was missing.' },
          { type: 'tool_use', id: 'toolu_test_qoderwork_1', name: 'Edit', input: { file_path: 'login.ts' } },
        ],
      },
    }),
    JSON.stringify({
      ...
      message: {
        ...
        content: [
          {
            type: 'tool_result',
            tool_use_id: 'toolu_test_qoderwork_1',
            content: 'File edited successfully',
            is_error: false,
          },
        ],
    ...
    JSON.stringify({
      ...
      timestamp: '2026-01-15T10:00:06.500Z',
    }),
    JSON.stringify({
      type: 'assistant',
      sessionId,
