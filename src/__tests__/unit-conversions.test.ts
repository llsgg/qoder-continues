...
function parseQoderFixtureMessages(filePath: string): ConversationMessage[] {
  const content = fs.readFileSync(filePath, 'utf8');
  const lines = content.trim().split('\n');
  const messages: ConversationMessage[] = [];
  ...
  for (const line of lines) {
    try {
      ...
      if (typeof blocks === 'string') {
        ...
        // Skip user lines that carry only tool_result blocks
        const hasHumanText = blocks.some((b: any) => b.type === 'text' && b.text);
        if (parsed.type === 'user' && !hasHumanText) continue;
        text = blocks
...
beforeAll(() => {
  ...
  fixtures.qoder = createQoderFixture();
  fixtures.qoderwork = createQoderWorkFixture();
  ...
  // codexcli shares the codex store and context extractor (pure CLI target).
  contexts.codexcli = {
    session: { ...codexSession, source: 'codexcli', id: '00000000-0000-0000-0000-000000000401' },
    recentMessages: codexMsgs,
    filesModified: [],
    pendingTasks: [],
    toolSummaries: [],
    markdown: generateHandoffMarkdown(
      { ...codexSession, source: 'codexcli', id: '00000000-0000-0000-0000-000000000401' },
      codexMsgs,
      [],
      [],
      [],
  ...
  // Qoder
  const qoderFile = fs
    .readdirSync(fixtures.qoder.root, { recursive: true })
    .map((f) => path.join(fixtures.qoder.root, f as string))
    .find((f) => f.endsWith('.jsonl'))!;
  const qoderSession: UnifiedSession = {
    id: 'task-test-qoder-1.session.execution',
    source: 'qoder',
    cwd: '/home/user/project',
    repo: 'user/project',
    lines: 6,
    bytes: fs.statSync(qoderFile).size,
    createdAt: now,
    updatedAt: now,
    originalPath: qoderFile,
    summary: 'Fix auth bug',
  };
  const qoderMsgs = parseQoderFixtureMessages(qoderFile);
  contexts.qoder = {
    session: qoderSession,
    recentMessages: qoderMsgs,
    filesModified: [],
    pendingTasks: [],
    toolSummaries: [],
    markdown: generateHandoffMarkdown(qoderSession, qoderMsgs, [], [], []),
  };

  // qodercli shares the qoder store and context extractor (pure CLI target).
  contexts.qodercli = {
    session: { ...qoderSession, source: 'qodercli', id: '00000000-0000-0000-0000-000000000301' },
    recentMessages: qoderMsgs,
    filesModified: [],
    pendingTasks: [],
    toolSummaries: [],
    markdown: generateHandoffMarkdown(
      { ...qoderSession, source: 'qodercli', id: '00000000-0000-0000-0000-000000000301' },
      qoderMsgs,
      [],
      [],
      [],
  ...
  // QoderWork
  const qoderWorkFile = fs
    .readdirSync(fixtures.qoderwork.root, { recursive: true })
    .map((f) => path.join(fixtures.qoderwork.root, f as string))
    .find((f) => f.endsWith('.jsonl'))!;
  const qoderWorkSession: UnifiedSession = {
    id: '00000000-0000-0000-0000-000000000201',
    source: 'qoderwork',
    cwd: '/home/user/project',
    repo: 'user/project',
    lines: 6,
    bytes: fs.statSync(qoderWorkFile).size,
    createdAt: now,
    updatedAt: now,
    originalPath: qoderWorkFile,
    summary: 'Fix auth bug',
    model: 'qwork-auto',
  };
  const qoderWorkMsgs = parseQoderFixtureMessages(qoderWorkFile);
  contexts.qoderwork = {
    session: qoderWorkSession,
    recentMessages: qoderWorkMsgs,
    filesModified: [],
    pendingTasks: [],
    toolSummaries: [],
    markdown: generateHandoffMarkdown(qoderWorkSession, qoderWorkMsgs, [], [], []),
  };
});
