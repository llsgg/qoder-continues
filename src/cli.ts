...
// Version resolution: read package.json when running from source/dist. In
// compiled single-file binaries (bun --compile) the virtual fs can't resolve
// ../package.json, so the release pipeline bakes the version in via
// `bun build --define process.env.CLI_HUB_VERSION=\"<version>\"`, which
// replaces this expression with a string literal at compile time.
const _require = createRequire(import.meta.url);
const VERSION = (() => {
  if (process.env.CLI_HUB_VERSION) return process.env.CLI_HUB_VERSION;
  try {
...
Quick start:
  $ continues
  $ npx qoder-continues --preset full
  $ qc claude 1

Core workflows:
  $ qc list
  $ qc list --source claude --limit 25
  $ qc list --jsonl | jq '.source'
  $ qc resume abc123
  $ qc resume abc123 --in gemini
  $ qc scan --rebuild

Inspect & export:
  $ qc inspect abc123 --preset full
  $ qc inspect abc123 --preset verbose --write-md handoff.md
  $ qc dump all ./out --preset verbose
  $ qc dump claude ./out --json --limit 50
...
Power tips:
  ...
  - Forward raw args to target tools after -- (example: qc claude 1 -- --help)
