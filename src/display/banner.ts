...
const FALLBACK_BANNER_LINE: RotatingBannerLine = () =>
  `${chalk.gray('💡 Tip:')} ${chalk.gray('Run')} ${chalk.cyan('qc --help')} ${chalk.gray(
...
const PRESET_PROMO_LINES: RotatingBannerLine[] = [
  () =>
    `${chalk.gray('🎛️ Preset:')} ${chalk.cyan('full')} ${chalk.gray(
      'is ideal for rich handoff context (npx qoder-continues --preset full).',
    )}`,
  () =>
...
const STAR_PROMO_LINES: RotatingBannerLine[] = [
  () =>
    `${chalk.bgHex('#FFD93D').black.bold(' 🙏 CREDITS ')} ${chalk.gray(
      'fork of')} ${chalk.hex('#00FFC8')('github.com/yigitkonur/cli-continues')}`,
];

const GENERAL_BANNER_LINES: RotatingBannerLine[] = [
  () =>
    `${chalk.gray('💡 Tip:')} ${chalk.gray('Use')} ${chalk.cyan('qc inspect <id> --preset full')} ${chalk.gray(
      'for maximum handoff detail.',
    )}`,
  () =>
    `${chalk.gray('💡 Tip:')} ${chalk.gray('Run')} ${chalk.cyan('qc dump all ./handoffs --preset verbose')} ${chalk.gray(
  ...
  () =>
    `${chalk.gray('🧠 Fact:')} ${chalk.gray(
      'The',
    )} ${chalk.cyan('qc')} ${chalk.gray('alias is built in — use')} ${chalk.cyan('qc claude')} ${chalk.gray(
  ...
  () =>
    `${chalk.gray('💡 Tip:')} ${chalk.gray('Run')} ${chalk.cyan('qc resume <id> --in gemini')} ${chalk.gray(
      'to hand off a Codex/Claude session cross-tool.',
    )}`,
  () =>
    `${chalk.gray('💡 Tip:')} ${chalk.gray('Use')} ${chalk.cyan('qc list --jsonl | jq')} ${chalk.gray(
      'when scripting filters around session metadata.',
    )}`,
  () =>
    `${chalk.gray('💡 Tip:')} ${chalk.gray('Try')} ${chalk.cyan('qc scan --rebuild')} ${chalk.gray(
      'if a freshly-created session is missing from the index.',
    )}`,
  () =>
    `${chalk.gray('💡 Tip:')} ${chalk.gray('Use')} ${chalk.cyan('qc inspect <id> --write-md handoff.md')} ${chalk.gray(
  ...
  () =>
    `${chalk.gray('🧠 Fact:')} ${chalk.gray('You can forward args to target CLIs after')} ${chalk.cyan('--')} ${chalk.gray(
      '(e.g. qc claude 1 -- --dangerously-skip-permissions).',
    )}`,
  () =>
  ...
  () =>
    `${chalk.gray('🧠 Fact:')} ${chalk.gray('Need quick routing?')} ${chalk.cyan('qc claude')} ${chalk.gray(
      'and',
    )} ${chalk.cyan('qc codex')} ${chalk.gray('jump straight into recent sessions.')}`,
];
...
export async function showBanner(version: string, supportsColor: boolean): Promise<boolean> {
  ...
  console.log(
    '  ' +
      ...
      chalk.cyan(`${ALL_TOOLS.length} AI coding agents`),
  );
  console.log();
  console.log(
    '  ' +
  ...
  console.log(
    '  ' +
      ...
      chalk.gray(' · ') +
      chalk.cyan('full') +
      chalk.gray(' (eg: npx qoder-continues --preset full for better context handoff!)'),
  );
  console.log(`  ${chalk.gray('💡 qc <tool> to quick-resume')}`);
