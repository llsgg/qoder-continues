...
export function formatSessionColored(session: UnifiedSession): string {
  ...
  // Session label: the source tool's native title when it has one
  // (registry-backed for qoder/qoderwork/codex), otherwise the derived summary
  const titleText = session.title || session.summary || '(no summary)';
  const summary = (session.title || session.summary ? chalk.white(titleText.slice(0, 44)) : chalk.gray(titleText)).padEnd(44);
...
export function formatSessionForSelect(session: UnifiedSession): string {
  ...
  const summary = (session.title || session.summary || '(no summary)').slice(0, 48);
