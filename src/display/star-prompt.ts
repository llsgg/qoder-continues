...
// Public repo of this fork — set once published on GitHub. Empty disables
// the star prompt (no silent repointing to the upstream repo).
const REPO = '';
...
export async function maybePromptGithubStar(): Promise<void> {
  ...
  const shouldStar = await clack.confirm({
    message: chalk.hex('#FFD93D')('⭐') + ' ' + chalk.gray('Enjoying qoder-continues? Star it on GitHub?'),
    initialValue: true,
  });
