...
import type { ConversationMessage, SessionContext, SessionParseOptions, SessionSource, UnifiedSession } from '../types/index.js';
...
export interface ToolAdapter {
   ...
   * doesn't have to paste anything. Receives the source session, the path of
   * the written handoff file, and the unified conversation; return null to
   * fall back to the generic flow (clipboard for GUI apps, prompt injection
   * for CLIs). GUI-app adapters surface the session themselves; CLI adapters
   * return `resumeArgs` and the caller spawns the binary with them instead
   * of the handoff prompt.
   */
  forgeHandoffSession?: (
    session: UnifiedSession,
    handoffPath: string,
    /** Unified conversation of the source session (full-preset extraction).
     *  Adapters use it as the cross-source fallback when the source
     *  transcript is not in the adapter's native line shape. */
    ...
    | {
        ...
        taskName: string;
        prepopulated: number;
        /** For CLI targets: argv to spawn instead of the handoff prompt
         *  (e.g. `['resume', '<forgedId>']`) — resumes the forged session
         *  losslessly. GUI-app adapters omit it. */
        resumeArgs?: string[];
        /** Optional hint: the target app was already running, so its in-memory
         *  views (e.g. a session-list catalog) may need a renderer reload to
         *  pick up the forged rows. Adapters without this caveat may omit it. */
        appWasRunning?: boolean;
        /** False when a renderer-reload attempt failed (e.g. missing macOS
         *  Accessibility permission for the terminal host). */
        refreshed?: boolean;
      }
    | null
  >;
...
// The codex family splits into two handoff targets:
// - codex: the Codex desktop app bundled inside ChatGPT.app. It shares
//   ~/.codex with the CLI — the app-server keeps a threads registry
//   (state_5.sqlite) pointing at the same rollout files — so the forge
//   writes a rollout + threads row and the session appears natively in the
//   app's list.
// - codexcli: the headless CLI/TUI; `codex resume <uuid>` continues sessions.
// Both share the sessions/ storage, so codex is the single session source
// (codexcli registers no discovery of its own).
register({
  name: 'codex',
  label: 'Codex',
  color: chalk.magenta,
  ...
  // flow when preconditions are missing.
  forgeHandoffSession: async (session, handoffPath, recentMessages) =>
    forgeCodexGuiHandoffSession(session, handoffPath, recentMessages),
  parseSessions: parseCodexSessions,
  extractContext: extractCodexContext,
  // Sessions stay resumable by the CLI binary regardless of the GUI target
  // (the desktop app has no external deep link for arbitrary sessions).
  nativeResumeArgs: (s) => ['resume', s.id],
  crossToolArgs: (prompt) => [prompt],
...
register({
  name: 'codexcli',
  label: 'Codex CLI',
  color: chalk.hex('#B98CD9'),
  storagePath: '~/.codex/sessions/',
  envVar: 'CODEX_HOME',
  binaryName: 'codex',
  // Full-auto handoff (qoder-family customization): forge a native rollout
  // carrying the source conversation and resume it (`codex resume <id>`) —
  // lossless per-message continuation instead of the prompt-injection blob.
  // Falls back to the prompt flow when no conversation/template is available.
  forgeHandoffSession: async (session, handoffPath, recentMessages) => {
    const forged = forgeCodexHandoffSession(session, handoffPath, recentMessages);
    return forged ? { ...forged, resumeArgs: ['resume', forged.chatId] } : null;
  },
  // Its sessions ARE the rollouts indexed by the codex adapter (shared
  // storage) — no session discovery of its own, purely a resume target.
  parseSessions: async () => [],
  extractContext: extractCodexContext,
  nativeResumeArgs: (s) => ['resume', s.id],
  crossToolArgs: (prompt) => [prompt],
...
// ── Qoder (New Qoder app + qodercli) ───────────────────────────────
// The qoder family splits into two handoff targets:
// - qoder: the New Qoder GUI app (com.qoder.app). Its sessions share the cosy
//   transcript layout under ~/.qoder/projects/<cwd-slug>/<uuid>.jsonl (the
//   legacy Qoder IDE layouts also live there and stay readable as sources).
// - qodercli: the headless CLI binary; `qodercli -r <uuid>` resumes sessions.
// CLI session ids are UUIDs; legacy IDE sessions (`task-*` ids) can only be
// resumed inside a GUI app.
const QODER_CLI_SESSION_ID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

register({
  name: 'qoder',
  label: 'Qoder',
  // Qoder (IDE + CLI + New Qoder app) writes transcripts under
  // <home>/projects/<cwd-slug>/ in two layouts:
  // <slug>/transcript/task-*.session.execution.jsonl (legacy IDE) and
  // <slug>/<uuid>.jsonl (CLI + New Qoder app). QODER_HOME overrides the
  // default ~/.qoder root.
  color: chalk.hex('#5B8DEF'),
  storagePath: '~/.qoder/projects/',
  envVar: 'QODER_HOME',
  binaryName: 'qodercli',
  ...
  // then bring the app to the foreground. Works whether the app is running
  // or not; falls back to clipboard handoff when preconditions are missing.
  forgeHandoffSession: forgeNewQoderHandoffSession,
  parseSessions: parseQoderSessions,
  supportsCwdLookup: true,
  extractContext: extractQoderContext,
  nativeResumeArgs: (s) => (QODER_CLI_SESSION_ID.test(s.id) ? ['-r', s.id] : []),
  crossToolArgs: (prompt) => [prompt],
  resumeCommandDisplay: (s) => (QODER_CLI_SESSION_ID.test(s.id) ? `qodercli -r ${s.id}` : 'qoder'),
});

register({
  name: 'qodercli',
  label: 'Qoder CLI',
  // Headless CLI target. Its sessions ARE the <uuid>.jsonl transcripts
  // indexed by the qoder adapter (shared storage), so it registers no
  // session discovery of its own — it exists purely as a cross-tool resume
  // target that spawns `qodercli <prompt>` in the terminal.
  color: chalk.hex('#7FA8F0'),
  storagePath: '~/.qoder/projects/',
  envVar: 'QODER_HOME',
  binaryName: 'qodercli',
  parseSessions: async () => [],
  ...
  nativeResumeArgs: (s) => (QODER_CLI_SESSION_ID.test(s.id) ? ['-r', s.id] : []),
  crossToolArgs: (prompt) => [prompt],
  resumeCommandDisplay: (s) => (QODER_CLI_SESSION_ID.test(s.id) ? `qodercli -r ${s.id}` : 'qodercli'),
});
...
register({
  name: 'qoderwork',
  label: 'QoderWork',
  // QoderWork is an Electron GUI agent without a CLI; sessions live as
  // <uuid>-session.json + <uuid>.jsonl pairs under the projects directory.
  // As a handoff target it uses the GUI-app path: clipboard + `open -a`.
  color: chalk.hex('#7C5CFF'),
  ...
  // (running app, db, MCP config) are missing.
  forgeHandoffSession: forgeQoderWorkHandoffSession,
  parseSessions: parseQoderWorkSessions,
  extractContext: extractQoderWorkContext,
  nativeResumeArgs: () => [],
  crossToolArgs: (prompt) => [prompt],
  resumeCommandDisplay: () => `qoderwork`,
});
